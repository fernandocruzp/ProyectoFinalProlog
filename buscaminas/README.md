...
Descripción:
    ...
    A travez de una cuadricula hay casillas que esperan ser "abiertas" 
    por el usuario, el objetivo del juego es abrir todas las casillas que no tengan minas 
    en su interior unicamente utilizando lógica.
    ...
    no hay minas a su alrededor esta no mostrara nada mas que cambiara su estado a "abierto".
...
Instrucciones de uso:
    ...
    El juego se basa en cordenadas, al ser una cuadricula de 8x8, los indices iran del 1 
    al 8 en ambos ejes ("x" y "y") de abajo hacia arriba y de izquierda a derecha respectivamente.
    
    Para realizar una consulta del tablero se va a implementar el comando:
        $ consulta(Tablero);

    El cuál imprimira el tablero con las casillas abiertas y los números en las casillas 
    ...
    Si al realizar una selección se selecciona una casilla con una mina, el jugador habra perdido 
    la partida, si al haber seleccionado una casilla vacia, esta liberara todas las casillas 
    vacias adyacentes de manera recursiva.

Reglas del juego:
    Deberas seleccionar casillas de manera "aleatoria" hasta que la información proporcionada por 
    las casillas en referencia a la posición de las minas te permita evadirlas al momento de hacer 
    una selección.

    El juego habra terminado al momento que el usuario haya abierto todas las casillas vacias, o 
...
Reglas logicas:
  ...
  1 |___|_1_|_1_|
    ...
    Las casillas vacias no han sido abiertas, en el caso de las casillas con un número, indican 
    que hay alguna mina en alguna de sus casillas vecinas a distancia 1 de ella misma, que en el 
    tablero de arriba nos indicaria que hay una mina en la casilla (2, 2)
       ___ ___ ___
    3 |___|___|___|
    2 |___|_1_|___|
    1 |___|___|___|
    ...
    En este caso, el tablero no nos da mucha información de donde podria haber una mina, podria 
    estar en alguna de las siguientes posiciones:
...
Ejemplo de sesión de juego:
  ...
  1 |___|___|_m_|___|___|_m_|___|___|
    ...
    Donde las casillas con una "m" tienen una mina en su interior, el usuario debera evadir las 
    minas a toda costa:
  ...
  1 |___|___|___|_1_|_2_|___|___|___|
    ...
    Liberamos las casillas que tienen un * las cuales no tienen minas a su alrededor, pero las 
    casillas con un número nos indican que hay un número n de minas a su alrededor.
